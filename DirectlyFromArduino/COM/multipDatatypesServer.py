...
def communicationWithGUI():
    ...
    while True:
        ...
        # Compressing image
        k = 10   # k = 5 -> 603kB k = 20 -> 37.6kB
        width = int((img.shape[1])/k)
        height = int((img.shape[0])/k)
        scaled = cv2.resize(img, (width, height), interpolation=cv2.INTER_AREA)

        # d = scaled
        # Creating dictionary with camera feed and other values
        d = {
            "key1": scaled,
            "key2": 232,
        ...
        while receiving:
            ...
            while receiving:
                ...
                if len(full_msg)-HEADERSIZE == msglen:
                    ...
                    print(response)
                    print(f'This is key 1 of the sent object {response["key1"]}')
...
j = 0
while True:
    # Just some other calculation and system processes
